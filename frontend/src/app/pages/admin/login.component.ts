...
@Component({
  ...
  <section class="section" style="min-height:100vh;display:grid;place-items:center;padding:32px;">
    <form class="card" style="width:min(460px,100%);padding:28px;" [formGroup]="form" (ngSubmit)="submit()">
      ...
      <div *ngIf="error" style="margin-top:14px;color:#ffd7d7;">{{ error }}</div>
      <div style="margin-top:20px;">
        <button class="btn btn-primary" style="width:100%;" [disabled]="form.invalid || loading">{{ loading ? 'Connecting...' : 'Login' }}</button>
      </div>
      <div class="muted" style="margin-top:16px;text-align:center;"></div>
    </form>
  </section>
  `
})
